          ...
          <div className="grid items-center gap-12 py-16 lg:grid-cols-[1.05fr_0.95fr] lg:gap-16 lg:py-24">
            ...
            <div>
              <span className="eyebrow text-white/55 before:bg-white/40">
                Location premium — Tunisie
              </span>

              <h1 className="mt-7 font-display text-[clamp(2.9rem,7vw,5.25rem)] font-medium leading-[0.98] tracking-[-0.03em]">
                La liberté
                <br />
                de la route,
                <br />
                <span className="italic text-white/45">sans limites.</span>
              </h1>
              ...
              <p className="mt-8 max-w-md text-base leading-relaxed text-white/65">
                ...
                assistance disponible à toute heure. Réservez en quelques minutes,
                conduisez l&apos;esprit tranquille.
              </p>

              <div className="mt-9 flex flex-col gap-3 sm:flex-row">
                <Link href="/voitures" className="btn-accent px-7 py-3.5">
                  Réservez maintenant
                  <Arrow />
                </Link>
                <Link href="#how" className="btn-ghost-dark px-7 py-3.5">
                  Comment ça marche
                </Link>
              </div>

              {/* Honest metadata row — no fabricated ratings */}
              <dl className="mt-12 grid max-w-md grid-cols-3 gap-px border-t border-white/12 pt-7 text-left">
                {[
                  { k: "Flotte", v: "Récente" },
                  { k: "Assistance", v: "24 / 7" },
                  { k: "Réponse", v: "< 30 min" },
                  ...
                  <div key={s.k}>
                    <dt className="text-[0.6rem] font-semibold uppercase tracking-[0.2em] text-white/40">
                      {s.k}
                    </dt>
                    <dd className="mt-1.5 font-display text-lg text-white">
                      {s.v}
                    </dd>
                  </div>
                ))}
            ...
            {/* Right — featured vehicle as an editorial plate */}
            <div className="hero-col-right">
              {hero ? (
                <figure className="group">
                  <div className="relative aspect-[4/3] overflow-hidden rounded-[var(--radius-lg)] border border-white/10 bg-cloud">
                    <span className="absolute left-4 top-4 z-10 inline-flex items-center gap-1.5 rounded-full bg-ink/80 px-2.5 py-1 text-[0.62rem] font-semibold uppercase tracking-[0.14em] text-white backdrop-blur-sm">
                      <span className="h-1.5 w-1.5 rounded-full bg-accent" />
                      Disponible
                    </span>
                    <span className="absolute right-4 top-4 z-10 rounded-full border border-ink/10 bg-paper/80 px-2.5 py-1 text-[0.62rem] font-medium uppercase tracking-[0.1em] text-stone backdrop-blur-sm">
                      {hero.category}
                    </span>
                    {hero.images?.[0] ? (
                      <Image
                      ...
                      <div className="flex h-full items-center justify-center">
                        <CarSilhouette className="h-20 w-20 text-ash" />
                      </div>
                    )}
                  </div>

                  <figcaption className="mt-6 flex items-end justify-between gap-6 border-t border-white/12 pt-6">
                    <div>
                      <p className="text-[0.62rem] font-semibold uppercase tracking-[0.24em] text-white/45">
                        {hero.brand}
                      </p>
                      <h2 className="mt-1.5 font-display text-2xl font-medium text-white">
                        {hero.name}
                      </h2>
                      <p className="mt-2 text-xs text-white/45">
                        {hero.transmission} · {hero.fuel_type} · {hero.seats} places
                      </p>
                    </div>
                    <div className="text-right">
                      ...
                      <p className="font-display text-3xl leading-none text-white">
                        {hero.price_per_day}
                        <span className="ml-1 text-sm text-white/45">DT</span>
                      </p>
                      <Link
                        href={`/voitures/${hero.id}`}
                        className="mt-3 inline-flex items-center gap-1.5 border-b border-white/30 pb-0.5 text-sm text-white transition-colors duration-200 hover:border-white"
                      >
                        Réserver <Arrow className="h-3 w-3" />
                      </Link>
                    </div>
                  </figcaption>
                </figure>
              ) : (
                <div className="rounded-[var(--radius-lg)] border border-white/12 p-8">
                  ...
                  <div className="mt-7 space-y-7">
                    {steps.slice(0, 3).map((s) => (
                      <div key={s.num} className="flex gap-5">
                        <span className="font-display text-2xl leading-none text-white/25">
                          {s.num}
                        </span>
                        <div>
